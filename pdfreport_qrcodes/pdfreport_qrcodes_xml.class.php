...
class pdfreport_qrcodes_xml
{
   ...
   function monta_xml()
   {
      ...
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['opcao'] = "";
      }
   }
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "pdfreport_qrcodes.xml";
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['embutida_tit'] . ">\r\n";
         }
         elseif ($this->New_Format)
         {
             $this->xml_registro = "<pdfreport_qrcodes>\r\n";
         }
         else
         {
             $this->xml_registro = "<pdfreport_qrcodes";
...
$_SESSION['scriptcase']['pdfreport_qrcodes']['contr_erro'] = 'off'; 
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->ticket))
         {
             $this->ticket = sc_convert_encoding($this->ticket, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <ticket>" . $this->trata_dados($this->ticket) . "</ticket>\r\n";
         }
         else
         {
             $this->xml_registro .= " ticket =\"" . $this->trata_dados($this->ticket) . "\"";
         }
   }
   //----- cliente
   function NM_export_cliente()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <cliente>" . $this->trata_dados($this->cliente) . "</cliente>\r\n";
         }
         else
         {
   ...
   function NM_export_telefone()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <telefone>" . $this->trata_dados($this->telefone) . "</telefone>\r\n";
         }
         else
         {
   ...
   function NM_export_qtde_cupons()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->qtde_cupons))
         {
             $this->qtde_cupons = sc_convert_encoding($this->qtde_cupons, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <qtde_cupons>" . $this->trata_dados($this->qtde_cupons) . "</qtde_cupons>\r\n";
         }
         else
         {
             $this->xml_registro .= " qtde_cupons =\"" . $this->trata_dados($this->qtde_cupons) . "\"";
         }
   }
   //----- imprimir
   function NM_export_imprimir()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <imprimir>" . $this->trata_dados($this->imprimir) . "</imprimir>\r\n";
         }
         else
         {
             $this->xml_registro .= " imprimir =\"" . $this->trata_dados($this->imprimir) . "\"";
         }
   }
   //----- created
   function NM_export_created()
   {
         ...
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->created))
         {
             $this->created = sc_convert_encoding($this->created, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         if ($this->New_Format)
         {
             $this->xml_registro .= " <created>" . $this->trata_dados($this->created) . "</created>\r\n";
         }
         else
         {
             $this->xml_registro .= " created =\"" . $this->trata_dados($this->created) . "\"";
         }
   }
   //----- qrcode
   function NM_export_qrcode()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <qrcode>" . $this->trata_dados($this->qrcode) . "</qrcode>\r\n";
         }
         else
         {
             $this->xml_registro .= " qrcode =\"" . $this->trata_dados($this->qrcode) . "\"";
         }
   }
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['pdfreport_qrcodes']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_chart_title'] ?> qrcodes :: XML</TITLE>
