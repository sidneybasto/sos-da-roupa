...
$old_error_handler = set_error_handler("vendedores_form_scriptcase_error_handler");

function vendedores_form_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['vendedores_form']['contr_erro']) && $_SESSION['scriptcase']['vendedores_form']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_vendedores_form->contr_vendedores_form->Apl_com_erro = true;
            if ($inicial_vendedores_form->contr_vendedores_form->NM_ajax_flag)
            {
                $inicial_vendedores_form->contr_vendedores_form->NM_ajax_info['errList']['geral_vendedores_form'][] = $msg;
                return;
            }
            vendedores_form_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function vendedores_form_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['vendedores_form'])  && '' != $_SESSION['scriptcase']['error_icon']['vendedores_form'])  ? $_SESSION['scriptcase']['error_icon']['vendedores_form']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['vendedores_form']) && '' != $_SESSION['scriptcase']['error_close']['vendedores_form']) ? $_SESSION['scriptcase']['error_close']['vendedores_form'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['vendedores_form']) && '' != $_SESSION['scriptcase']['error_span_title']['vendedores_form']) ? $_SESSION['scriptcase']['error_span_title']['vendedores_form'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['vendedores_form']) && '' != $_SESSION['scriptcase']['error_icon_title']['vendedores_form']) ? $_SESSION['scriptcase']['error_icon_title']['vendedores_form'] : '';
?>
